...
export default class AboutYouPage {
  ...
  async updateEmployment() {
    ...
    await this.page
      .getByTestId("div-equal-opportunities-form-section")
      .getByRole("textbox", { name: "Industry" })
      .click();
    await this.page
      .getByTestId("div-equal-opportunities-form-section")
      .getByRole("textbox", { name: "Industry" })
    ...
    await this.page
      .getByRole("textbox", { name: "Knowledge Gained*" })
      .fill("test");
  ...
  async addEducation() {
    ...
    await this.page
      .getByRole("textbox", { name: "University/College*" })
      .click();
    await this.page
      .getByRole("textbox", { name: "University/College*" })
      .fill("punjab university");
    await this.page.getByLabel("Sandwich Course").selectOption("Yes");
    await this.page
      .getByRole("textbox", { name: "Graduation Start Year [e.g." })
      .click();
    await this.page
      .getByRole("button", { name: "Date 23 March" })
      .first()
      .click();
    await this.page.locator("#addQualificationEndDateField").click();
    await this.page
      .getByRole("button", { name: "Date 28 March" })
      .nth(1)
      .click();
    ...
    await this.page
      .getByRole("textbox", { name: "Course/Degree Subject" })
      .click();
    await this.page
      .getByRole("textbox", { name: "Course/Degree Subject" })
      .fill("a");
    ...
    await this.page
      .getByRole("textbox", { name: "Level/Type of Degree *" })
      .click();
    await this.page
      .getByRole("textbox", { name: "Level/Type of Degree *" })
      .fill("2022");
    ...
    await this.page
      .getByRole("cell", { name: "Level/Type of Degree * Answer" })
      .getByLabel("Level/Type of Degree *")
      .selectOption("Obtained");
  ...
  async uploadCV(fileName: string) {
    await this.page
      .getByTestId("label-choose-cv-file-0")
...
//data extraction fill data extract data
  async extractPersonalDetails() {
    const address = await this.page.getByTestId("txt-txtCURRENTADDRESS").inputValue();
    const city = await this.page.getByTestId("txt-txtCITYTOWN").inputValue();
    const county = await this.page.getByTestId("txt-txtCOUNTY").inputValue();
    const postcode = await this.page.getByTestId("txt-txtPOSTCODE").inputValue();
    const relocate = await this.page.getByTestId("select-txtRELOCATE").inputValue();
    const termCity = await this.page.getByTestId("txt-txtTERMCITY").inputValue();
    const termCounty = await this.page.getByTestId("txt-txtTERMCOUNTY").inputValue();
