...
```
Error: locator.textContent: Error: strict mode violation: locator('.datepicker:visible').locator('.datepicker-switch') resolved to 5 elements:
    1) <th colspan="5" class="datepicker-switch">July 2026</th> aka getByRole('columnheader', { name: 'July' })
    2) <th colspan="5" class="datepicker-switch">2026</th> aka locator('th').filter({ hasText: /^2026$/ })
    3) <th colspan="5" class="datepicker-switch">2020-2029</th> aka getByText('-2029')
    4) <th colspan="5" class="datepicker-switch">2000-2090</th> aka getByText('-2090')
    5) <th colspan="5" class="datepicker-switch">2000-2900</th> aka getByText('-2900')

Call log:
  - waiting for locator('.datepicker:visible').locator('.datepicker-switch')
        ...
        - generic [ref=e38]:
          - generic [ref=e39]:
            ...
            - generic [ref=e96]:
              ...
              - textbox "Candidate Primary Email" [ref=e98]: ixwq2lw7mk@gmail.com
            - generic [ref=e101]:
...
```ts
  ...
  13 |  * <div class="datepicker datepicker-dropdown-menu datepicker-orient-right datepicker-orient-bottom">).
  14 |  *
  15 |  * @param trigger      locator for the input that opens the picker
  16 |  *                      e.g. page.getByRole('textbox', { name: 'Date From' })
  17 |  * @param day           target day, e.g. "20" (must match cell text exactly)
  18 |  * @param monthYear     target header text, e.g. "March 2024"
  19 |  * @param direction     'next' (default) or 'prev', depending whether the
  20 |  *                      target date is after or before what's shown on open
  21 |  */
  22 | export async function selectDate(
  23 |   page: Page,
  24 |   trigger: ReturnType<Page['locator']>,
  25 |   day: string,
  26 |   monthYear: string,
  27 |   direction: 'next' | 'prev' = 'next',
  28 |   maxClicks = 24
  29 | ) {
  30 |   await trigger.click();
  31 | 
  32 |   const popup = page.locator('.datepicker:visible');
  33 |   await popup.waitFor({ state: 'visible' });
  34 | 
  35 |   const header = popup.locator('.datepicker-switch');
  36 |   const navBtn = popup.locator(direction === 'next' ? '.next' : '.prev');
  37 |   // :not(.old):not(.new) excludes days from adjacent months that bleed
  38 |   // into the current month's grid, so "1" doesn't match the wrong month.
  39 |   const exactDayCell = (label: string) =>
  40 |     popup.locator('td.day:not(.old):not(.new)', { hasText: new RegExp(`^${label}$`) });
  41 | 
  42 |   for (let i = 0; i < maxClicks; i++) {
> 43 |     const currentHeader = (await header.textContent())?.trim();
     |                                         ^ Error: locator.textContent: Error: strict mode violation: locator('.datepicker:visible').locator('.datepicker-switch') resolved to 5 elements:
  44 |     if (currentHeader === monthYear) {
  45 |       await exactDayCell(day).click();
  46 |       return;
  47 |     }
  ...
  50 | 
  51 |   throw new Error(
  52 |     `selectDate: could not reach "${monthYear}" after ${maxClicks} "${direction}" clicks — verify .datepicker-switch/.next/.prev/td.day selectors against the real widget`
  53 |   );
  54 | }
```