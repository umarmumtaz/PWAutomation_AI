...
export default defineConfig({
  ...
  fullyParallel: false, // Disable parallel execution for sequential flow
...
reporter: [
  ['html', { open: 'always' }],              // Playwright’s built-in HTML report
  ['json', { outputFile: 'test-results.json' }], // JSON output
  ['allure-playwright']                      // Allure reporter 
  // run the alure report results   npx allure serve allure-results
],
  /* Shared settings for all the projects below. See https://playwright.dev/docs/api/class-testoptions. */
 use: {
   /* Base URL to use in actions like `await page.goto('')`. */
   baseURL: 'https://test.jobtrain.co.uk/voyagecare/',
   headless: true,
   screenshot: "on",
   launchOptions: {
     slowMo: 2000
